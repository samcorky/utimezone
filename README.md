...
- **MicroPython Optimised:** Avoids heavy objects and `datetime` dependencies; uses simple tuples and epoch integers.
...
| `local_datetime_to_utc(*dt)`       | Converts local naive date/time to a UTC time tuple.                   |
| `local_to_utc_epoch(*dt)`          | Resolves local naive date/time to a UTC epoch integer.                |
...
When a local time occurs twice due to a DST end transition (e.g., 01:30 am happens before and after the shift), `utimezone` returns the **earlier UTC instant**.
